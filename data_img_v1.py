...
def all_objects(img_d, labels, name, id_img):
    ...
    try:
        if labels.shape[0] >= 2:
            ...
            limx1, limx2, limy1, limy2, x_ch, y_ch = crop_limits(p1x_l, p1y_l, p2x_l, p2y_l, x_aumt, y_aumt, ancho,
                                                                 alto)
            ...
            for descr, aug in augs:
                images_aug = aug.augment_images([img_resize] * 1)

            cv2.imwrite(path_save_images + name + '_all_b_aug' + '.jpg', images_aug[0])
            doc1 = open(path_save_labels + name + '_all_b_aug' + '.txt', 'w')
